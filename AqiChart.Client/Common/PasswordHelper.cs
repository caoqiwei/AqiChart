...
namespace AqiChart.Client.Common
{
    public class PasswordHelper
    {
        ...
        private static void OnPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            VisibilityPlaceholder(password);
        }
        ...
        private static void Password_PasswordChanged(object sender, RoutedEventArgs e)
        {
            ...
            _isUpdating = false;
            VisibilityPlaceholder(passwordBox);
        }

        private static string TagData = "";
        private static void VisibilityPlaceholder(PasswordBox passwordBox)
        {
            if (string.IsNullOrEmpty(TagData))
            {
                TagData = passwordBox.Tag?.ToString();
            }
            if (!string.IsNullOrEmpty(TagData))
            {
                if (string.IsNullOrEmpty(passwordBox.Password))
                    passwordBox.Tag = TagData;
                else
                    passwordBox.Tag = "";
            }
        }
